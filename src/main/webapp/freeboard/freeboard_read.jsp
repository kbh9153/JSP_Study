...
<head>
<meta charset="EUC-KR">
<title>Column의 특정 레코드를 읽는 페이지</title>
</head>
<body>
<%
	String sql = null;
	...
	// if (true) return;	// 프로그램을 여기서 멈춤 (디버깅 시에 많이 사용됨)
	...
	try {
		...
		if (!rs.next()) {	// DB에 해당 값이 없는 경우
			out.println("데이터베이스에 해당 내용이 없습니다.");
		} else {	// DB에 해당 값이 있는 경우
			// out.println("데이터베이스에 해당 내용이 존재합니다.");
			String em = rs.getString("email");
			if ((em != null && (!(em.equals(""))))) {	// DB의 email 컬럼의 값이 있
