...
<BODY>
				...
				<TD align=right width=70% valign=bottom>
					<FONT size=2 face=굴림>
						<SELECT name=stype>
							...
							int what = 1; // what : stype에서 전송받은 변수를 저장한 변수
							// 1 > 이름
							// 2 > 제목
							// 3 > 내용
							// 4 > 이름+제목
							...
							if (request.getParameter("stype") != null) {
								...
								if (what == 1) {	// 1 > 이름 만 검색
									out.println("<OPTION value=1 selected>이름");		// list 창에서 이름을 검색시 search 창에서 이름검색 선택되어 있음
									cond = " where name like '%" + val + "%'";	// 쿼리문. 와일드카드(like) DB의 name 컬럼을 검색
								} else {
	...
	<CENTER>
		<TABLE border=0 width=600 cellpadding=4 cellspacing=0>
         ...
         int totalgroup = 0;
         int maxpages = 2;	// 페이지그룹별 최대 페이지 수 (화면에 출력할 페이지 개수)
         int startpage = 1;
         ...
         try {
            ...
            String sql = "select * from freeboard " + cond;	// cond에 검색어가 담긴 검색옵션이 들어가있음
